...
function localFreshDataApi(apiKey: string | undefined, model: string | undefined, ticketmasterApiKey: string | undefined): Plugin {
    ...
    configureServer(server) {
      server.middlewares.use(async (request, response, next) => {
        ...
        if (ingestionCity) {
          ...
          const snapshot = await refreshCityEvents({ cityId: ingestionCity, ticketmasterApiKey, previous });
        ...
        if (!apiKey) {
          ...
          response.end(JSON.stringify(pathname.includes("/events/") ? verifiedSanFranciscoFallback() : unavailablePulse()));
          return;
        }
        try {
          const now = Date.now();
          if (pathname.includes("/events/")) {
            if (!eventsCache || eventsCache.expiresAt <= now) {
              eventsCache = { expiresAt: now + CACHE_MS, payload: verifiedSanFranciscoFallback() };
            ...
            response.end(JSON.stringify(eventsCache.payload));
          } else {
...
export default defineConfig(({ mode }) => {
  ...
  return {
    plugins: [react(), localFreshDataApi(env.XAI_API_KEY, env.XAI_MODEL, env.TICKETMASTER_API_KEY)],
