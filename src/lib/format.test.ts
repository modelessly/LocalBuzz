...
import { formatDateTimeRange, formatDateTimeRangeAccessible, formatTimeRange, priceLabel } from "./format";
...
describe("city-local date and time formatting", () => {
  ...
  it("marks estimated endings and includes the next local date after midnight", () => {
    ...
    expect(formatDateTimeRange("2026-09-03T23:30:00-07:00", "2026-09-04T01:00:00-07:00", "America/Los_Angeles", true)).toBe("SEP 3 · 11:30 PM–SEP 4 · 1:00 AM (EST.)");
  });
  ...
  it("provides a full accessible description and truthful unknown-price copy", () => {
    const description = formatDateTimeRangeAccessible("2026-09-03T20:00:00+02:00", "2026-09-03T21:30:00+02:00", "Europe/Stockholm", true);
    expect(description).toMatch(/Thursday.*3 September 2026.*20:00.*21:30.*End time estimated/i);
    expect(priceLabel(undefined, "SEK")).toBe("Price unavailable");
  });
});
