...
import { formatDateTimeRange, priceLabel } from "../lib/format";
  ...
  return <section id="discovery-review" className="discovery-review" aria-labelledby="discovery-review-title">
    ...
    <div className="discovery-review__grid">{options.map((lead) => {
      return <article key={lead.id} className="discovery-lead">
        ...
        <p className="discovery-lead__source"><a href={lead.originalSourceUrl} target="_blank" rel="noopener noreferrer">View source <ExternalLink aria-hidden="true" size={12} /></a></p>
        <dl className="discovery-lead__facts">{lead.leadType === "event" ? <><div><dt>When</dt><dd>{lead.fields.timing?.start ? <time dateTime={lead.fields.timing.start}>{formatDateTimeRange(lead.fields.timing.start, lead.fields.timing.end, timeZone)}</time> : "Time to be confirmed"}</dd></div><div><dt>Where</dt><dd>{lead.fields.venue?.name ?? "Location to be confirmed"}{lead.fields.venue?.address ? ` · ${lead.fields.venue.address}` : ""}</dd></div><div><dt>Category</dt><dd>{lead.fields.category?.replaceAll("_", " ") ?? "Other"}</dd></div>{lead.fields.commerce?.currency ? <div><dt>Price</dt><dd>{priceLabel(lead.fields.commerce.priceMin, lead.fields.commerce.currency)}</dd></div> : null}</> : <><div><dt>Kind</dt><dd>{lead.fields.kind?.replaceAll("_", " ") ?? "Place"}</dd></div><div><dt>Where</dt><dd>{lead.fields.location?.address ?? "Location to be confirmed"}</dd></div>{lead.fields.priceRange ? <div><dt>Price</dt><dd>{priceLabel(lead.fields.priceRange.min, lead.fields.priceRange.currency)}</dd></div> : null}</>}</dl>
